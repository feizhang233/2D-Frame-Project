...
def calculate_normal_stress(element, axial_force, moment_y, moment_z, y, z):
    ...
    finite("section y", y)
    finite("section z", z)
    return np.asarray(axial_force)/element.A + np.asarray(moment_y)*z/element.Iy - np.asarray(moment_z)*y/element.Iz


def calculate_element_field_results(element, node_i, geometry, local_displacements,
                                   local_end_forces, intensities, *, number_of_points=101,
                                   deformation_scale=1.0, section_points=(), local_stiffness=None):
    d, s, L = local_displacements, local_end_forces, geometry.L
    loads = [P([intensities[0, i], (intensities[1, i]-intensities[0, i])/L])
             for i in range(4)]
    ...
    energy_density = (n*n/(element.E*element.A) + tx*tx/(element.G*element.J)
                      + my*my/(element.E*element.Iy) + mz*mz/(element.E*element.Iz))
    ...
    line_work = sum(q*a for q, a in zip(loads, [u, v, w, rx])).integ()(L)
    ...
    energy_ratio, energy_error, energy_roundoff = energy_check(
        strain_energy, boundary_work+line_work, d, local_stiffness,
    ...
    force_scale = max(1.0, np.max(np.abs(s[[0, 1, 2, 6, 7, 8]])),
                      L*np.max(np.abs(intensities[:, :3])))
    moment_scale = max(1.0, np.max(np.abs(s[[3, 4, 5, 9, 10, 11]])), force_scale*L,
                       L*np.max(np.abs(intensities[:, 3])))
    normalized = max(np.max(np.abs(action_error[:3]))/force_scale,
    ...
    passed = (normalized <= 1e-8 and release_error/moment_scale <= 1e-8
    ...
    validation = ElementEquilibriumValidation(
        action_error[:3].tolist(), action_error[3:].tolist(), float(normalized),
        translation_error, rotation_error, release_error, energy_ratio,
        energy_error, energy_roundoff, bool(passed))
    ...
    stress = np.array([calculate_normal_stress(element, values[6], values[10], values[11], p.y, p.z)
                       for p in section_points]).reshape(len(section_points), len(x))
