...
from .models import Node, FrameElement, Support, NodalLoad, DistributedLoad, SectionPoint, finite
...
from .assembly import calculate_element_dof_map, assemble_nodal_load_vector
from .loads import local_load_intensities, equivalent_load_from_intensities
from .releases import condense_releases
from .solution import solve_system
from .recovery import recover_element_end_response
from .postprocessing import ElementFieldResults, ElementEquilibriumValidation, calculate_element_field_results
...
def solve_frame(nodes, elements, supports, nodal_loads=(), distributed_loads=(), *,
                number_of_points=101, deformation_scale=1.0, section_points=()):
    ...
    with np.errstate(over="raise", invalid="raise", divide="raise"):
        try:
            return _solve_frame(nodes, elements, supports, nodal_loads, distributed_loads,
                                number_of_points, deformation_scale, section_points)
        except (FloatingPointError, OverflowError) as error:
            raise ValueError("numerical overflow; check coordinates, properties and load scales") from error


def _solve_frame(nodes, elements, supports, nodal_loads, distributed_loads,
                 number_of_points, deformation_scale, section_points):
    ...
    for e, geom, kl, t, fl, condensed, intensities in matrices:
        ...
        fields, validation, energy, line_work = calculate_element_field_results(
            e, by_node[e.node_i], geom, end.local_displacements, end.local_end_forces, intensities,
            number_of_points=number_of_points, deformation_scale=deformation_scale,
            section_points=section_points, local_stiffness=kl)
        distributed_work += line_work
        recovered.append(ElementAnalysisResult(e.id, geom, end.local_displacements,
                                               end.local_end_forces, fields, validation, energy))
    ...
    moment_scale = max(1.0, np.abs(external[:, 3:]).sum()+np.abs(support[:, 3:]).sum()
    ...
    energy_ratio, energy_error, energy_roundoff = energy_check(
        energy, work, d, k, float((np.abs(fnodal)+np.abs(reactions)) @ np.abs(d))+abs(distributed_work))
    ...
    symmetry = float(np.linalg.norm(khat-khat.T)/max(np.finfo(float).tiny, np.linalg.norm(khat)))
    passed = all(e.validation.passed for e in recovered) and max(
        symmetry, force_ratio, moment_ratio, balance_f, balance_m, energy_ratio) <= 1e-8
    validation = GlobalValidation(symmetry, max(force_ratio, moment_ratio), force_ratio,
                                  moment_ratio, force_balance.tolist(), moment_balance.tolist(),
                                  balance_f, balance_m, float(energy), work, energy_ratio,
                                  energy_error, energy_roundoff,
                                  solution.scaled_condition_number, bool(passed))
    return FrameAnalysisResult(d, d.reshape(-1, 6), reactions, free, solution.restrained_dofs,
                               solution.inactive_dof_vectors, solution.active_dof_count,
                               tuple(recovered), validation)
