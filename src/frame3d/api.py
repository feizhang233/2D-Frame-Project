...
from .models import Node, FrameElement, Support, NodalLoad, DistributedLoad, SectionPoint, DOF_NAMES, FORCE_NAMES
from .solver import solve_frame
from .plotting import COMPONENTS, render_internal_force_plot, png_data_uri
...
def _solve(payload):
    return solve_frame(
        ...
        number_of_points=payload.number_of_points, deformation_scale=payload.deformation_scale,
...
def solve(payload: SolveRequest):
    ...
    if payload.include_plots:
        plots = {name: {"filename": name+".png", "media_type": "image/png",
                        "data_uri": png_data_uri(render_internal_force_plot(
                            result.elements, name, dpi=payload.plot_dpi))} for name in COMPONENTS}
    return {
        "displacement_dof_order": "global [u,v,w,rx,ry,rz] per node; support partitions use support axes",
        "force_dof_order": "[Fx,Fy,Fz,Mx,My,Mz] per node",
        "section_force_convention": "A00 positive-x face [N,Vy,Vz,Tx,My,Mz]; s_local=k_local*d_local-f_local",
        "field_recovery": "equilibrium-integrated prismatic beam fields including supported load particular solutions",
        "free_dofs": result.free_dofs.tolist(), "restrained_dofs": result.restrained_dofs.tolist(),
        "inactive_dof_vectors": result.inactive_dof_vectors.tolist(),
        "active_dof_count": result.active_dof_count,
        "nodal_displacements": [{"node_id": i+1, **dict(zip(DOF_NAMES, row))}
                                for i, row in enumerate(result.nodal_displacements)],
        "nodal_reactions": [{"node_id": i+1, **dict(zip(FORCE_NAMES, row))}
                            for i, row in enumerate(result.reactions.reshape(-1, 6))],
        "section_points": [p.model_dump() for p in payload.section_points],
        "elements": [{"element_id": e.element_id, "length": e.geometry.L,
                      ...
                      "strain_energy": e.strain_energy, "validation": asdict(e.validation)}
                     for e in result.elements],
        "validation": asdict(result.validation), "plots": plots,
    }


ForceComponent = Literal["axial_force", "shear_force_y", "shear_force_z",
                         "torsional_moment", "bending_moment_y", "bending_moment_z"]


@router.post("/plots/{component}", response_class=Response,
             responses={200: {"content": {"image/png": {}}}})
def force_plot(component: ForceComponent, payload: SolveRequest):
    result = _solve(payload)
    return Response(render_internal_force_plot(result.elements, component, dpi=payload.plot_dpi),
                    media_type="image/png",
                    headers={"Content-Disposition": f'inline; filename="{component}.png"'})


app = FastAPI(title="frame3d API", version="0.1.0",
...
def run():
    import uvicorn
    uvicorn.run("frame3d.api:app", host=os.environ.get("FRAME3D_HOST", "0.0.0.0"),
