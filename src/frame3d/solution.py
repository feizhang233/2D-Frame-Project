...
from .models import identifier, array
...
def support_data(number_of_nodes, supports):
    ...
    for support in supports:
        ...
        for dof, flag, value in zip(dofs, support.restraints, support.prescribed_values):
            if flag:
...
def solve_system(stiffness, load, supports, *, rotational_connectivity=None):
    ...
    for node in range(n//6):
        ...
        directions = (np.eye(3) if rotational_connectivity is None
    ...
    if null.shape[1]:
        ...
        if np.any(np.abs(projected) > 1e-9*np.maximum(1.0, scales)):
            raise ValueError("load acts on an inactive released joint rotation; no equilibrium exists")
    condition = 1.0
    ds = dc.copy()
    if b.shape[1]:
        reduced = b.T @ ks @ b
        ...
        if eigenvalues[0] <= 1e-12*eigenvalues[-1]:
            raise ValueError("singular or ill-conditioned K_ff: check supports, releases and connectivity")
    ...
    return LinearSolution(d, k @ d-f, np.flatnonzero(~fixed), np.flatnonzero(fixed),
                          (s.T @ null).T, b.shape[1], condition, s)
...
def calculate_reaction_vector(global_stiffness, displacements, total_load):
    ...
    n = len(k)
    return (array("global_stiffness", k, (n, n)) @ array("displacements", displacements, (n,))
