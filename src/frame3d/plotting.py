...
def render_internal_force_plot(elements, component, *, dpi=140):
    ...
    with _LOCK:
        ...
        for element in elements:
            axes.plot(element.fields.x_local, getattr(element.fields, component),
                      label=f"Element {element.element_id}")
        axes.axhline(0, color="#777777", linewidth=.8)
        axes.set(title=f"Space frame: {symbol} (positive-x section)",
                 xlabel="Local x [m]", ylabel=f"{symbol} [{unit}]")
        axes.grid(alpha=.25)
        axes.legend()
        output = BytesIO()
