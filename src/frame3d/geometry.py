...
def calculate_geometry(element: FrameElement, node_i: Node, node_j: Node,
                       *, length_tolerance=1e-12, direction_tolerance=1e-10):
    ...
    q = np.array([ex, np.cos(angle)*ey + np.sin(angle)*ez,
                  -np.sin(angle)*ey + np.cos(angle)*ez])
