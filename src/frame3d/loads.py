...
import numpy as np
from .models import positive, array
...
def equivalent_load_from_intensities(length, intensities):
    ...
    for point, weight in zip(points, weights):
        ...
        h = np.array([1-3*t*t+2*t**3, L*(t-2*t*t+t**3),
                      3*t*t-2*t**3, L*(-t*t+t**3)])
        factor = weight*L/2
...
def calculate_local_equivalent_nodal_load(element, load, length, *, geometry=None):
    ...
    return equivalent_load_from_intensities(length, local_load_intensities(element, [load], geometry))
