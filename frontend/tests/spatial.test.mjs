...
import { applyMaterial, applySection, autoReference, blankModel, defaultSection, deleteSelected, example3D, insertMember, insertNode, modelIssues, parseModel3D, planeCoords, planePoint, toSpatialPayload, xyz } from '../src/spatial/model.ts'
import { basis, project, unproject } from '../src/spatial/projection.ts'
...
test('repository examples import through the same frontend parser', () => {
  for (const name of ['cantilever_3d', 'space_frame_3d']) {
    const model = parseModel3D(JSON.parse(readFileSync(new URL(`../../examples/${name}.json`, import.meta.url), 'utf8')))
...
test('frontend payload solves through the strict backend API schema, including a connected subdivided beam', () => {
  ...
  const python = spawnSync(new URL('../../.venv/bin/python', import.meta.url).pathname, ['-c', `import json,sys
...
`], { input: JSON.stringify(toSpatialPayload(split)), encoding: 'utf8', env: { ...process.env, MPLCONFIGDIR: '/private/tmp/frame3d-mpl-cache', XDG_CACHE_HOME: '/private/tmp/frame3d-cache' } })
