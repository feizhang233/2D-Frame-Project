...
export type Node3D = { id: number; x: number; y: number; z: number }
export type Member = {
  id: number; node_i: number; node_j: number; E: number; G: number; A: number
  Iy: number; Iz: number; J: number; reference_vector: Vec3; roll_angle: number
  theory: 'euler_bernoulli' | 'timoshenko'; Asy?: number; Asz?: number; releases: number[]
}
export const dofs = ['u', 'v', 'w', 'rx', 'ry', 'rz'] as const
export const forces = ['fx', 'fy', 'fz', 'mx', 'my', 'mz'] as const
export type Support = { node_id: number; axes?: number[][] } & Partial<Record<typeof dofs[number], boolean>> & Partial<Record<`${typeof dofs[number]}_value`, number>>
...
export type Result3D = {
  nodal_displacements: ({ node_id: number } & Record<typeof dofs[number], number>)[]
  nodal_reactions: NodalLoad[]
  elements: { element_id: number; length: number; local_axes: number[][]; local_end_forces: number[]
    local_displacements: number[]; fields: Record<string, number[]> & { normal_stress: number[][] } }[]
  validation: { passed: boolean; force_balance_ratio: number; moment_balance_ratio: number; energy_residual_ratio: number; scaled_condition_number: number; free_dof_residual_ratio: number }
}
...
export function parseModel3D(raw: unknown): Model3D {
  ...
  model.supports = array('supports').map(s => {
    ...
    if (s.axes !== undefined && s.axes !== null) {
      ...
      support.axes = s.axes.map(row => (row as unknown[]).map(v => finite(v, 'Support axis')))
...
export function toSpatialPayload(model: Model3D) {
  return {
