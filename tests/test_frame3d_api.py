...
from frame3d.api import app as standalone
...
def payload():
    return {
        ...
        "elements": [{"id": 1, "node_i": 1, "node_j": 2, "E": 200e9, "G": 80e9,
                      "A": .01, "Iy": 8e-6, "Iz": 5e-6, "J": 1e-5, "reference_vector": [0, 1, 0]}],
        "supports": [{"node_id": 1, "u": True, "v": True, "w": True, "rx": True, "ry": True, "rz": True}],
        "nodal_loads": [{"node_id": 2, "fy": 1000, "fz": 1000, "mx": 2000}],
        "section_points": [{"y": .05, "z": 0}],
        "number_of_points": 5,
        "include_plots": False,
    }


@pytest.mark.parametrize("application,path", [(app, "/api/v1/3d/solve"), (standalone, "/api/v1/solve")])
def test_solve_response_contract(application, path, payload):
    ...
    assert body["nodal_displacements"][1]["rx"] == pytest.approx(.005)
...
@pytest.mark.parametrize("mutation", ["missing_z", "unknown_field", "negative_J", "bool_number", "parallel_axis",
                                      "zero_length", "no_supports", "translation_support_only", "unknown_node",
                                      "missing_shear_area", "timoshenko_line", "bad_release", "singular_release",
                                      "conflicting_settlement", "loaded_inactive", "oversized_sampling", "overflow"])
def test_invalid_models_return_422(mutation, payload):
    element = payload["elements"][0]
    if mutation == "missing_z": del payload["nodes"][1]["z"]
    if mutation == "unknown_field": payload["nonlinear"] = True
    if mutation == "negative_J": element["J"] = -1
    if mutation == "bool_number": element["E"] = True
    if mutation == "parallel_axis": element["reference_vector"] = [1, 0, 0]
    if mutation == "zero_length": payload["nodes"][1]["x"] = 0
    if mutation == "no_supports": payload["supports"] = []
    if mutation == "translation_support_only": payload["supports"] = [{"node_id": 1, "u": True, "v": True, "w": True}]
    if mutation == "unknown_node": element["node_j"] = 3
    if mutation == "missing_shear_area": element["theory"] = "timoshenko"
    if mutation == "timoshenko_line":
        element.update(theory="timoshenko", Asy=.008, Asz=.008)
        payload["distributed_loads"] = [{"element_id": 1, "qy_i": 1000}]
    if mutation == "bad_release": element["releases"] = [7]
    if mutation == "singular_release": element["releases"] = [3, 9]
    if mutation == "conflicting_settlement": payload["supports"].append({"node_id": 1, "u": True, "u_value": .001})
    if mutation == "loaded_inactive":
        element["releases"] = [11]
        payload["nodal_loads"] = [{"node_id": 2, "mz": 100}]
    if mutation == "oversized_sampling": payload["number_of_points"] = 2002
    if mutation == "overflow": element.update(E=1e308, A=1e308)
...
def test_timoshenko_and_settlement_are_exposed(payload):
    payload["elements"][0].update(theory="timoshenko", Asy=.008, Asz=.008)
    payload["supports"][0]["v_value"] = .001
    result = TestClient(app).post("/api/v1/3d/solve", json=payload)
    assert result.status_code == 200
    body = result.json()
    assert body["nodal_displacements"][1]["v"] == pytest.approx(.001+8e3/3e6+2000/(80e9*.008))
    assert body["validation"]["passed"]
...
def test_plots_and_direct_png_endpoint(payload):
    ...
    assert set(plots) == {"axial_force", "shear_force_y", "shear_force_z", "torsional_moment",
...
def test_openapi_and_2d_schema_are_kept_separate(payload):
    ...
    assert "/api/v1/solve" in schema["paths"]
