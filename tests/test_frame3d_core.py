...
E, G, A, IY, IZ, J, L = 200e9, 80e9, .01, 8e-6, 5e-6, 1e-5, 2.0
...
def rotation():
    ...
    return np.eye(3)+np.sin(.73)*skew+(1-np.cos(.73))*(skew@skew)
...
@pytest.mark.parametrize("end,reference", [((0, 0, 0), (0, 1, 0)), ((2, 0, 0), (1, 0, 0)),
                                           ((2, 0, 0), (1, 1e-12, 0))])
def test_v00_degenerate_geometry(end, reference):
    with pytest.raises(ValueError):
        f.calculate_geometry(replace(ELEMENT, reference_vector=reference), NODES[0], f.Node(2, *end))
...
def test_v01_independent_strain_displacement_integral():
    ...
    for point, weight in zip(points, weights):
        ...
        b[0, [0, 6]] = [-1/L, 1/L]
        b[1, [3, 9]] = [-1/L, 1/L]
        h2 = np.array([(-6+12*t)/L**2, (-4+6*t)/L,
                       (6-12*t)/L**2, (-2+6*t)/L])
        b[2, [1, 5, 7, 11]] = h2
...
def test_v02_six_rigid_modes_and_scaled_rank(theory):
    e = replace(ELEMENT, theory=theory, Asy=.008, Asz=.008)
...
@pytest.mark.parametrize("force,dofs,expected,energy", [
    ({"fx": 10000}, [0], [1e-5], .05),
    ({"mx": 2000}, [3], [.005], 5),
...
def test_v07_pure_bending_and_normal_stress():
    r = cantilever(nodal=[f.NodalLoad(2, mz=1000)], section_points=[f.SectionPoint(.05, 0)])
    fields = r.elements[0].fields
    close(fields.transverse_displacement_y, .001*fields.x_local**2/2, atol=1e-15)
    close(fields.rotation_z, .001*fields.x_local, atol=1e-15)
    close(fields.normal_stress, -1e7)
    assert r.validation.strain_energy == pytest.approx(1)
    assert f.calculate_normal_stress(ELEMENT, 10000, 2000, 3000, .04, -.02) == pytest.approx(
        10000/A + 2000*(-.02)/IY - 3000*.04/IZ)
...
def test_v08_uniform_load_and_particular_solution():
    ...
    close(r.nodal_displacements[1, [1, 5]], [.002, .004/3])
    close(r.elements[0].local_end_forces[6:], 0, atol=1e-8)
    assert r.elements[0].fields.bending_moment_z[1] == pytest.approx(500)
    assert r.elements[0].fields.transverse_displacement_y[1] == pytest.approx(17*1000*L**4/(384*E*IZ))
...
def test_uniform_and_triangular_fields_against_virtual_work(component):
    for qi, qj in [(1000, 1000), (0, 1000), (700, -300)]:
        ...
        fields = r.elements[0].fields
        name, rigidity = {"qx": ("axial_displacement", E*A), "qy": ("transverse_displacement_y", E*IZ),
                          "qz": ("transverse_displacement_z", E*IY), "mx": ("rotation_x", G*J)}[component]
...
def test_fixed_fixed_load_has_nonzero_internal_displacement_and_energy():
    r = f.solve_frame(NODES, [ELEMENT], [FIXED, replace(FIXED, node_id=2)],
                      distributed_loads=[f.DistributedLoad(1, qy_i=1000, qy_j=1000)], number_of_points=3)
    close(r.displacements, 0)
    assert r.elements[0].fields.transverse_displacement_y[1] == pytest.approx(1000*L**4/(384*E*IZ))
...
def test_v09_support_settlement_and_duplicate_records():
    ...
    supports = [FIXED, f.Support(2, v=True, w=True, rx=True, ry=True, rz=True),
                replace(FIXED, node_id=3, u_value=.002), f.Support(1, u=True)]
    r = f.solve_frame(nodes, elements, supports)
    close(r.nodal_displacements[:, 0], [0, .001, .002])
    close(r.reactions[[0, 12]], [-2e6, 2e6])
    assert r.validation.passed
    with pytest.raises(ValueError, match="conflicting"):
        f.solve_frame(nodes, elements, supports+[f.Support(1, u=True, u_value=.001)])
...
def test_v10_rotation_covariance_with_mixed_loading():
    ...
    rotated = cantilever(element=replace(e, reference_vector=tuple(h@e.reference_vector)),
                         nodes=[f.Node(n.id, *(h@n.coordinates)) for n in nodes],
                         nodal=[f.NodalLoad(2, *(h@force), *(h@moment))], line=line)
...
def test_v12_release_condenses_load_and_recovers_member_rotation():
    ...
    load = f.DistributedLoad(1, qy_i=1000, qy_j=1000)
    c = f.condense_releases(f.calculate_local_stiffness(e, L),
                            f.calculate_local_equivalent_nodal_load(e, load, L), e.releases)
    close(c.load[[1, 5, 7]], [1250, 500, 750])
    r = f.solve_frame(NODES, [e], [FIXED, f.Support(2, u=True, v=True, w=True, rx=True, ry=True)],
...
def test_rotated_released_joint_null_direction_and_loaded_inactive_error():
    ...
    r = f.solve_frame(nodes, [e], supports, distributed_loads=[f.DistributedLoad(1, qy_i=1000, qy_j=1000)])
    assert r.validation.passed
    assert r.inactive_dof_vectors.shape == (1, 12)
...
def test_both_end_bending_releases_simply_supported_beam():
    ...
    r = f.solve_frame(NODES, [e], [replace(FIXED, rz=False), f.Support(2, v=True)],
                      distributed_loads=[f.DistributedLoad(1, qy_i=1000, qy_j=1000)], number_of_points=3)
    close(r.reactions[[1, 7]], [-1000, -1000])
    close(r.elements[0].local_end_forces[[5, 11]], 0, atol=1e-8)
    assert r.elements[0].fields.transverse_displacement_y[1] == pytest.approx(5*1000*L**4/(384*E*IZ))
    assert r.validation.passed
...
def test_v13_timoshenko_both_planes_and_eb_limit(axis, inertia, rotation_dof, sign):
    e = replace(ELEMENT, theory="timoshenko", Asy=.008, Asz=.008)
    force = np.zeros(3)
    force[axis] = 1000
    r = cantilever(element=e, nodal=[f.NodalLoad(2, *force)])
    assert r.nodal_displacements[1, axis] == pytest.approx(1000*L**3/(3*E*inertia)+1000*L/(G*.008))
    assert r.nodal_displacements[1, rotation_dof] == pytest.approx(sign*1000*L**2/(2*E*inertia))
    close(f.calculate_local_stiffness(replace(e, Asy=1e12, Asz=1e12), L),
          f.calculate_local_stiffness(ELEMENT, L), rtol=1e-12)
...
def test_v14_inclined_member_assembly(count):
    ...
    elements = [replace(ELEMENT, id=i+1, node_i=i+1, node_j=i+2, reference_vector=(0, 0, 1))
                for i in range(count)]
    q = f.calculate_geometry(elements[0], *nodes[:2]).Q
    r = f.solve_frame(nodes, elements, [FIXED], [f.NodalLoad(count+1, *(q.T@np.array([0, 1000, 0])))])
...
def test_v17_triangular_consistent_load_and_tip_response():
    ...
    close(r.nodal_displacements[1, [1, 5]], [11*1000*L**4/(120*E*IZ), 1000*L**3/(8*E*IZ)])


def test_v18_locking_diagnostic_and_closed_element_pure_bending_energy():
    curvature, shear_area = .001, .008
    gp, gw = np.polynomial.legendre.leggauss(3)
    x = (gp+1)*L/2
    bad_shear = .5*G*shear_area*np.dot(gw, (curvature*(L/2-x))**2)*L/2
    good_bending = .5*E*IZ*curvature**2*L
    assert bad_shear == pytest.approx(213.3333333333333)
    assert good_bending == pytest.approx(1)
    d = np.zeros(12)
    d[[7, 11]] = [curvature*L**2/2, curvature*L]
    for area in (.008, 8e3):
        k = f.calculate_local_stiffness(replace(ELEMENT, theory="timoshenko", Asy=area, Asz=area), L)
        assert .5*d@k@d == pytest.approx(good_bending)
...
def test_inclined_support_with_prescribed_displacement():
    ...
    support = replace(FIXED, axes=tuple(map(tuple, h.T)), u_value=.001)
    ...
    close(h.T@r.nodal_displacements[1, :3], [.001+1000*L/(E*A), 0, 0], atol=1e-13)
    assert r.validation.passed
...
def test_planar_frame_matches_existing_2d_solver():
    ...
    supports2 = [planar.Support(1, True, True, True, u_value=.0001, angle=20)]
    ...
    elements3 = [replace(ELEMENT, reference_vector=(-1, 0, 0)),
                 replace(ELEMENT, id=2, node_i=2, node_j=3)]
    angle = np.deg2rad(20)
    axes = ((np.cos(angle), np.sin(angle), 0), (-np.sin(angle), np.cos(angle), 0), (0, 0, 1))
    supports3 = [replace(FIXED, u_value=.0001, axes=axes),
                 f.Support(2, w=True, rx=True, ry=True), f.Support(3, w=True, rx=True, ry=True)]
    b = f.solve_frame(nodes3, elements3, supports3, [f.NodalLoad(3, fx=1200, fy=-1500, mz=500)],
...
@pytest.mark.parametrize("changes", [dict(J=-1), dict(G=0), dict(E=float("nan")), dict(node_j=1),
                                      dict(releases=(11, 11)), dict(releases=(1,)), dict(releases=(True,)),
                                      dict(reference_vector=(0, 0, 0)), dict(theory="unknown"),
                                      dict(theory="timoshenko"), dict(Asy=-1), dict(roll_angle=float("inf"))])
def test_invalid_element_inputs(changes):
    with pytest.raises((ValueError, TypeError)):
        replace(ELEMENT, **changes)


@pytest.mark.parametrize("case", ["missing_node", "duplicate_node", "duplicate_element", "isolated_node",
                                   "unknown_nodal_load", "unknown_line_load", "unknown_support",
                                   "timoshenko_line_load", "invalid_sampling"])
def test_model_errors(case):
    nodes, elements, supports, nodal, line = list(NODES), [ELEMENT], [FIXED], [], []
    kwargs = {}
    if case == "missing_node": elements = [replace(ELEMENT, node_j=3)]
    if case == "duplicate_node": nodes.append(NODES[1])
    if case == "duplicate_element": elements.append(ELEMENT)
    if case == "isolated_node": nodes.append(f.Node(3, 3, 3, 3))
    if case == "unknown_nodal_load": nodal.append(f.NodalLoad(3))
    if case == "unknown_line_load": line.append(f.DistributedLoad(3))
    if case == "unknown_support": supports.append(replace(FIXED, node_id=3))
    if case == "timoshenko_line_load":
        elements = [replace(ELEMENT, theory="timoshenko", Asy=.008, Asz=.008)]
        line = [f.DistributedLoad(1, qy_i=1)]
    if case == "invalid_sampling": kwargs["number_of_points"] = True
    with pytest.raises((ValueError, TypeError)):
...
def test_load_superposition_and_unsorted_input_ids():
    ...
    b = cantilever(nodes=NODES[::-1], nodal=[f.NodalLoad(2, fy=400), f.NodalLoad(2, fy=600)],
                   line=[f.DistributedLoad(1, qz_j=40), f.DistributedLoad(1, qz_j=60)])
    close(a.displacements, b.displacements, atol=1e-14)


def test_invalid_support_bases_and_values():
    for kwargs in [dict(u=False, u_value=1), dict(u="yes"), dict(axes=((1, 0, 0), (0, 1, 0), (0, 0, -1)))]:
        with pytest.raises((ValueError, TypeError)):
            replace(FIXED, **kwargs)
    with pytest.raises(ValueError, match="same axes"):
        f.solve_frame(NODES, [ELEMENT], [FIXED, f.Support(1, u=True, axes=tuple(map(tuple, rotation())))])
...
def test_prescribed_rigid_motion_does_not_fail_energy_due_to_roundoff():
    ...
    for _ in range(5):
        ...
        nodes = [f.Node(n.id, *(h@n.coordinates)) for n in NODES]
        e = replace(ELEMENT, reference_vector=tuple(h@np.array([0, 1, 0])))
        translation, omega = rng.normal(size=3)*.003, rng.normal(size=3)*.002
        supports = [f.Support(n.id, True, True, True, True, True, True,
                              *(translation+np.cross(omega, n.coordinates)), *omega) for n in nodes]
...
def test_energy_diagnostic_still_rejects_material_work_mismatch():
    from frame3d.validation import energy_check
    ratio, error, bound = energy_check(1., 3., np.ones(12)*.001, np.eye(12)*1e6, 3.)
    assert ratio > .3
    assert error == 1
    assert bound < 1e-10
...
def test_nearly_parallel_connections_are_not_misclassified_as_inactive():
    from frame3d.solution import solve_system
    stiffness = np.diag([1., 1., 1., 1., 1e-14, 1.])
    directions = [[(1., 0., 0.), (0., 0., 1.), (0., 1e-7, 1.)]]
    force = np.array([0., 0., 0., 0., 1., 0.])
    r = solve_system(stiffness, force, [f.Support(1, u=True, v=True, w=True)],
                     rotational_connectivity=directions)
